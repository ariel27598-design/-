...
export default function ShareLibraryModal({ name, url, cloudStatus = 'idle', onClose }: Props) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4" onClick={onClose}>
      <div
        className="flex w-full max-w-sm flex-col gap-4 rounded-3xl border border-white/10 bg-ink-900 p-5"
        onClick={(e) => e.stopPropagation()}
      >
        <h2 className="text-lg font-bold text-white">{t('shareLinkTitle', { name })}</h2>
        <p className="text-xs text-ink-400">{t('shareLinkHint')}</p>

        {qrDataUrl && (
          <img src={qrDataUrl} alt="QR" className="mx-auto h-52 w-52 rounded-xl bg-white p-2" />
        )}

        <div className="break-all rounded-xl border border-white/10 bg-white/5 px-3 py-2 text-xs text-ink-300" dir="ltr">
          {url}
        </div>
        ...
        <div className="flex gap-2">
          <button onClick={copyLink} className="flex-1 rounded-full bg-primary-500 py-2.5 text-sm font-bold text-white hover:bg-primary-400">
