  ...
  const filtered = useMemo(() => {
    return games
      .filter((g) => {
        ...
        if (query.trim() && !tText(g.name).toLowerCase().includes(query.trim().toLowerCase())) return false;
        return true;
      })
      .sort((a, b) => tText(a.name).localeCompare(tText(b.name)));
  }, [games, ownedFilter, activeCategories, cooperativeOnly, query, tText]);
    ...
    <div className="flex flex-col gap-5">
      ...
      <div className="flex flex-wrap gap-2">
        {(
          [
            ['all', t('filterAll')],
            ['owned', t('filterOnShelf')],
            ['missing', t('filterNotOnShelf')],
